...
class MAMI_Dataset(Base_Dataset):
    def __init__(self, split: str, **kargs):
        super(MAMI_Dataset, self).__init__()
        self.data_path = Path('data/MAMI')
        self.img_path = self.data_path / 'img'
        self.data = self.get_data(split)
    
    def get_data(self, split: str) -> pd.DataFrame:
        data = pd.read_json('data/MAMI/data.jsonl', orient='records', lines=True)
    ...
    def get_img(self, mid: str) -> Image.Image:
        img_path = self.img_path / f'{mid}.jpg'
